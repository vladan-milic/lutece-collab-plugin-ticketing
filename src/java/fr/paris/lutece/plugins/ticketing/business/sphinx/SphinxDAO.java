...
//    		+ " ) AS Y";
	...
	private static final String SQL_SELECT_DOMAINS_LIST = "	SELECT Group_concat(T2.label SEPARATOR ';') AS 'Domaines' " + 
			...
			"			) AS parent_id, " + 
			"			@l := @l + 1 AS lvl " + 
			"		FROM( SELECT @r := ?, @l := 0) vars CROSS " + 
			"		JOIN ticketing_category m " + 
			"		WHERE @r <> 0 " + 
			"	) T1 " + 
