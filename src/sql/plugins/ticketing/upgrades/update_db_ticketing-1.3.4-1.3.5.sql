...
UPDATE site_ticketing.ticketing_ticket_domain d,
...
(SELECT *  from site_ticketing.ticketing_ticket_domain d WHERE d.inactive <> 1 ORDER BY d.id_ticket_type, d.id_ticket_domain ) x) dom
...
-- init of Domain orders with increasing sequences grouped by ticket_type
UPDATE site_ticketing.ticketing_ticket_category c,
...
(SELECT *  from site_ticketing.ticketing_ticket_category c WHERE c.inactive <> 1 ORDER BY c.id_ticket_domain, c.id_ticket_category ) x) cat
